...
namespace GoogleMobileAds.Editor {
  public static class PListProcessor {
    ...
    private const string SKADNETWORKS_RELATIVE_PATH =
...
#pragma warning restore 0168
    ...
    private static void AddSKAdNetworkIdentifier(PlistDocument document,
      ...
      if (array != null) {
        ...
        NotifyBuildFailure(
            "SKAdNetworkItems element already exists in Info.plist, but is not an array.", false);
      }
    }

    private static bool ContainsSKAdNetworkIdentifier(PlistElementArray skAdNetworkItemsArray,
                                                      string id) {
...
#pragma warning restore 0168
    ...
    private static void NotifyBuildFailure(string message, bool showOpenSettingsButton = true) {
      ...
      if (showOpenSettingsButton) {
        bool openSettings =
            EditorUtility.DisplayDialog(dialogTitle, dialogMessage, "Open Settings", "Close");
        if (openSettings) {
