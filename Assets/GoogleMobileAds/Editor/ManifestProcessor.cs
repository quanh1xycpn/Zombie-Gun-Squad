...
#endif
  {
...
#endif
    {
      ...
      if (pathUtils.IsPackageRootPath()) {
        ...
        string packagesPathPrefix = Path.GetDirectoryName(pathUtils.GetParentDirectoryAssetPath());
      ...
      if (!File.Exists(manifestPath)) {
        manifestPath =
            Path.Combine(Path.GetDirectoryName(manifestPath), "src", "main", "AndroidManifest.xml");
      }
...
#pragma warning restore 0168
      ...
      IEnumerable<XElement> metas =
          elemApplication.Descendants().Where(elem => elem.Name.LocalName.Equals("meta-data"));

      SetMetadataElement(elemApplication, metas, METADATA_APPLICATION_ID, appId);

      SetMetadataElement(elemApplication, metas, METADATA_OPTIMIZE_INITIALIZATION,
                         !instance.DisableOptimizeInitialization, true);

      SetMetadataElement(elemApplication, metas, METADATA_OPTIMIZE_AD_LOADING,
                         !instance.DisableOptimizeAdLoading, true);
    ...
    private XElement CreateMetaElement(string name, object value) {
      return new XElement("meta-data", new XAttribute(ns + "name", name),
    ...
    private XElement GetMetaElement(IEnumerable<XElement> metas, string metaName) {
      foreach (XElement elem in metas) {
        ...
        foreach (XAttribute attr in attrs) {
          if (attr.Name.Namespace.Equals(ns) && attr.Name.LocalName.Equals("name") &&
              attr.Value.Equals(metaName)) {
            return elem;
          }
        }
      }
    ...
    private void SetMetadataElement(XElement elemApplication, IEnumerable<XElement> metas,
    ...
    private void SetMetadataElement(XElement elemApplication, IEnumerable<XElement> metas,
                                    string metadataName, bool metadataValue,
